...
export class OrchestratorService {
  ...
  constructor(
    ...
    if (!apiKey) {
      this.logger.warn(
        'GEMINI_API_KEY is not configured. LLM features will be disabled.',
      );
    } else {
  ...
  public async startDiagnostic(connectionId: string): Promise<void> {
    ...
    try {
      const response: any = await firstValueFrom(
        this.httpService.get(connection.adapterUrl),
      );
      const data = response.data;

      if (this.isOpenApiSpec(data)) {
        this.logger.log(
          `Processing OpenAPI spec for connection ${connectionId}`,
        );
        connection.draftSpecification = data;
        connection.status = ConnectionStatus.PENDING_VALIDATION;
      } else if (this.genAI) {
        this.logger.log(
          `Using LLM to process documentation for connection ${connectionId}`,
        );
        ...
        throw new Error(
          'Data is not a valid OpenAPI spec and LLM is not configured.',
        );
      }
      ...
      this.logger.error(
        `Failed to diagnose connection ${connectionId}`,
        error.stack,
      );
  ...
  private async generateSchemaFromDocumentation(
    documentation: any,
  ): Promise<any> {
